...
8. atlas 路径必须是安全相对路径，动画行列不能越过 atlas，interaction 必须引用现有状态，capability 值必须是布尔值。
9. Import Folder 只能包含普通文件和目录，任何层级的 symlink 都会被拒绝。

桌面端预览针对选择时复制出的不可变 snapshot 执行。用户在预览后修改原文件夹，不会改变确认导入的内容；关闭设置、取消、新选目录、退出应用或超过 30 分钟都会清理 pending snapshot。
...
- 如果启动时出现自动修复提示，先在设置页打开 Imported Pet Quarantine，修复原始宠物包后重新导入，不要直接把隔离目录移回管理目录。
