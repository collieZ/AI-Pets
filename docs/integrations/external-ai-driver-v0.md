...
桌面端在“设置 > 测试工具 > 外部 AI 事件桥”中展示最近的成功与失败调用，包含时间、来源、HTTP 状态码和事件摘要，便于接入 Codex、Cursor 等工具时排查参数问题。

- 日志仅保存在主进程内存中，不写入磁盘，也不会记录请求以外的系统数据。
- 最多保留最近 100 条，超过后自动移除最早的记录。
- 可以在设置面板中手动清空，应用完全退出后也会自动清空。
